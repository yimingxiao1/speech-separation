...
class AudioDataset(data.Dataset):
    ...
    def __init__(self, json_dir, batch_size, sample_rate=8000, segment=2.0, cv_maxlen=8.0):
        """
        Args:
        ...
        mix_json = os.path.join(json_dir, 'mix_new.json')
        s1_json = os.path.join(json_dir, 's1_new.json')
        s2_json = os.path.join(json_dir, 's2_new.json')
        ...
        if segment >= 0.0:
            ...
            for _, sample, _ in sorted_mix_infos:
...
def load_mixtures_and_sources(batch):
    ...
    for mix_info, s1_info, s2_info in zip(mix_infos, s1_infos, s2_infos):
        mix_path = mix_info[0]#.replace('/media/zhang/SpiderMan/Conv-TasNet-master','..')#'/media/zhang/SpiderMan' /media/xiao/xizo/home
        s1_path = s1_info[0]#.replace('/media/zhang/SpiderMan/Conv-TasNet-master','..')
        s2_path = s2_info[0]#.replace('/media/zhang/SpiderMan/Conv-TasNet-master','..')
